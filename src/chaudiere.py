...
class StrUtils:
   ...
   * @param stringToStrip the string whose trailing characters are to be removed
...
class StringTokenizer:
   ...
   '''
   * Constructs a new instance of StringTokenizer using a string with tokens and a delimiter
...
class KeyValuePairs:
   ...
   * @return boolean indicating whether the key exists
   '''
   def hasKey(self, key):
      return self.mapKeyValues.has_key(key)
...
class IniReader:
   ...
   * @return the sectionName surrounded by square brackets
   ...
   def bracketedSection(sectionName):
      return IniReader.OPEN_BRACKET + StrUtils.strip(sectionName) + IniReader.CLOSE_BRACKET
   ...
   * @return boolean indicating if the file was successfully read
   ...
   def readFile(self):
      ...
      while strippingComments:
         posCommentStart = self.fileContents.find(IniReader.COMMENT_IDENTIFIER, posCurrent)
         if -1 == posCommentStart:
            # not found
            strippingComments = 0
         else:
            posCR = self.fileContents.find(IniReader.EOL_CR, posCommentStart + 1)
            posLF = self.fileContents.find(IniReader.EOL_LF, posCommentStart + 1)
            haveCR = (-1 != posCR)
            haveLF = (-1 != posLF)
   ...
   * @return the value for the requested key, or None on error
   ...
   '''
   * Reads the specified section from the configuration and populates the passed KeyValuePairs instance
   * @param sectionName the name of the section whose configuration values to retrieve
   * @param mapSectionValues the KeyValuePairs instance to populate with configuration data
   * @return boolean indicating whether configuration data could be retrieved for the section
   * @see KeyValuePairs()
   '''
   def readSection(self, sectionName, mapSectionValues):
      sectionId = IniReader.bracketedSection(sectionName)
      ...
      startNextSection = self.fileContents.find(IniReader.OPEN_BRACKET, posEndSection)
      ...
      while parsingSectionContents:
         ...
         if len(line) > 0:
            ...
            if (posEqual != -1) and (posEqual < len(line)):
               ...
               if not StrUtils.startsWith(key, IniReader.COMMENT_IDENTIFIER):
                  mapSectionValues.addPair(key, StrUtils.strip(line[posEqual+1:]))
